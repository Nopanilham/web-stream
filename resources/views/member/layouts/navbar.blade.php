...
<div class="flex justify-between items-center">
    ...
    <div class="flex items-center gap-[26px]">
        <span class="text-white text-base">Selamat Datang, {{ auth()->user()->name }}</span>
        <!-- user avatar -->
        <div class="collapsible-dropdown flex flex-col gap-2 relative">
            ...
            <div class="bg-white rounded-2xl text-stream-dark font-medium flex flex-col gap-1 absolute z-[999] right-0 top-[80px] min-w-[180px] hidden overflow-hidden"
                id="dropdown-stream">
                <a href="{{ route('member.dashboard') }}" class="transition-all hover:bg-sky-100 p-4">Menonton</a>
                <!-- <a href="#!" class="transition-all hover:bg-sky-100 p-4">Settings</a> -->
                <a href="{{ route('member.logout') }}" class="transition-all hover:bg-sky-100 p-4">Keluar</a>
            </div>
        </div>
    </div>
</div>
